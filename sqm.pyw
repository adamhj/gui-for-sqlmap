...
class app(Frame):
    def __init__(self, mw):
        ...
        self.nRoot.add(BuilderFrame, text='SQLmap Command Builder')
        self.nRoot.add(WatchLog, text='Log viewer')
        self.nRoot.add(Editor, text='Editor')
        self.nRoot.add(HelpMe, text='Help!')
        ...
        #
        trafbut = ttk.Button(butPanel, width=10)
        ...
        #
        urlLF = ttk.Labelframe(panedUrl, text='target:')
        ...
        rbLOG = ttk.Radiobutton(targetVariant, text='logFile', variable=self.varTarget, value="logFile",
                                command=self.fTarget)
        rbBULKFILE = ttk.Radiobutton(targetVariant, text='bulkFile', variable=self.varTarget, value="bulkFile",
                                     command=self.fTarget)
        rbREQUEST = ttk.Radiobutton(targetVariant, text='requestFile', variable=self.varTarget, value="requestFile",
                                    command=self.fTarget)
        rbDork = ttk.Radiobutton(targetVariant, text='googleDork', variable=self.varTarget, value="googleDork",
                                 command=self.fTarget)
        rbDirect = ttk.Radiobutton(targetVariant, text='direct', variable=self.varTarget, value="direct",
                                   command=self.fTarget)
        rbConfig = ttk.Radiobutton(targetVariant, text='configFile', variable=self.varTarget, value="configFile",
                                   command=self.fTarget)
        ...
        # query to sqlmap
        queryLF = ttk.Labelframe(panedUrl, text='query to sqlmap:')
        ...
        self.noBF.add(setingsF, text='Settings')
        self.noBF.add(sDetTechF, text='Injection | Detection | Technique')
        ...
        #
        self.eCrawl = ttk.Entry(genOptLF, width=10)
        self.eCrawl.grid(row=1, column=5, sticky='w', padx=3)
        # --csv-del=CSVDEL
        self.eCsv = ttk.Entry(genOptLF, width=10)
        ...
        self.box['values'] = (
        "access", "db2", "firebird", "maxdb", "mssqlserver", "mysql", "oracle", "postgresql", "sqlite", "sybase")
        self.box.current(0)
        ...
        self.chkRandomAg.config(text="random-agent", variable=self.chkRandomAg_var, onvalue="on", offvalue="off",
                                command=self.fRandomAg)
...
def main():
    root = Tk()
    f = tkFont.Font(family='Helvetica', size=8)
    s = ttk.Style()
    s.theme_use('clam')
    s.configure('.', font=f)
